...
Class Stacey {
  ...
  static $version = '2.3.0-smiletous';
  ...
  function __construct($get) {
	...
	# begin special redirection case for smiletous
	if(!file_exists($file_path)){
		$url_parts = explode('/', $this->route);
		array_unshift($url_parts, 'tours');
		$url = implode('/', $url_parts);
		
		if(file_exists(Helpers::url_to_file_path($url))){
			header('HTTP/1.1 301 Moved Permanently');
			header('Location:../'.$url);
			return;
		}
	}
	# end special redirection case
